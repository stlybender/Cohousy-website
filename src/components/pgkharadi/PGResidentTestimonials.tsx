...
import { REVIEWS, GOOGLE_RATING } from './reviewsData'
...
function Avatar({ src, name }: { src: string; name: string }) {
  ...
  return (
    <span className="relative block h-11 w-11 shrink-0">
      <span className="flex h-11 w-11 items-center justify-center overflow-hidden rounded-full bg-accent text-lg font-bold text-black">
        {failed || !src ? (
...
export default function PGResidentTestimonials() {
    ...
    >
      <div className="container mx-auto px-6">
        ...
        >
          ...
          >
            What Residents Say
            <span className="text-accent"> on Google</span>
          </motion.h2>
          ...
          >
            Not testimonials we wrote — real Google reviews, published by residents under
            their own names. Every one is clickable, so you can verify it yourself.
          </motion.p>
        </motion.div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <a
                ...
                className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-accent text-black font-semibold rounded-lg hover:shadow-lg transition-all duration-300"
              >
        ...
        >
          {REVIEWS.map((review) => (
            ...
            >
              <div className="flex items-start gap-3 mb-4">
                <Avatar src={review.photo} name={review.name} />
                ...
                <div className="min-w-0">
                  <a
                    ...
                    className="font-bold text-black hover:text-accent transition-colors duration-300 block truncate"
                  >
                    {review.name}
                  </a>
          ...
          <motion.a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="group bg-white rounded-2xl border border-dashed border-gray-300 p-6 flex flex-col items-center justify-center text-center hover:border-accent transition-all duration-500"
          >
            <GoogleLogo size={32} className="mb-3" />
            <div className="font-bold text-black group-hover:text-accent transition-colors duration-300">
              + {GOOGLE_RATING.count - REVIEWS.length} more reviews
            </div>
            <p className="text-sm text-gray-500 mt-1">Read them all on Google Maps</p>
            <ExternalLink
              size={16}
              className="text-gray-400 group-hover:text-accent transition-colors duration-300 mt-3"
            />
          </motion.a>
        </motion.div>
